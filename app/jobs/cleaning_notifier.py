...
def format_cleaning_message(bookings: List[Booking], recipient: Any) -> str:
    ...
    msg_header = (
        f"🧹 <b>План уборки на ЗАВТРА ({target_date.strftime('%d.%m')})</b>\n\n"
    )
    msg_body = ""
...
def format_cleaning_keyboard(
    bookings: List[Booking], recipient: Any
        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="✅ Подтверждаю",
                    callback_data=f"cleaner:confirm:{target_date}",
                ),
                InlineKeyboardButton(
                    text="❌ Не смогу", callback_data=f"cleaner:decline:{target_date}"
                ),
            ]
        ]
    )
