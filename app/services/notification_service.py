...
class NotificationRule:
    ...
    keyboard_func: Optional[Callable[[List[Booking], Any], Optional[InlineKeyboardMarkup]]] = None
...
class NotificationService:
    ...
    async def _notify_cleaners(self, session, bookings: List[Booking], rule: NotificationRule):
        ...
        for cleaner in cleaners:
            try:
                ...
                keyboard = rule.keyboard_func(bookings, cleaner) if rule.keyboard_func else None
    ...
    async def _notify_guests(self, session, bookings: List[Booking], rule: NotificationRule):
        ...
        for booking in bookings:
            ...
            if target_user:
                try:
                    ...
                    keyboard = rule.keyboard_func([booking], target_user) if rule.keyboard_func else None
                    ...
                    count += 1
                    logger.info(f"Notified guest {target_user.name} ({target_user.telegram_id}) for booking {booking.id}")
                except Exception as e:
                    logger.error(f"Failed to notify guest {target_user.telegram_id}: {e}")
            else:
                logger.debug(f"Guest user not found for booking {booking.id} (phone {booking.guest_phone})")
                
        logger.info(f"Rule {rule.name}: Sent to {count} guests.")

    async def _notify_admins(self, session, bookings: List[Booking], rule: NotificationRule):
        """Уведомление админов"""
        ...
        for admin_id in admin_ids:
            try:
                text = rule.message_func(bookings, None) # None = нет персонализации получателя
                keyboard = rule.keyboard_func(bookings, None) if rule.keyboard_func else None
