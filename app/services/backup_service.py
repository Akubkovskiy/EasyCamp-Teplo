...
async def restore_latest_backup():
    ...
    if os.path.exists(db_path) and os.path.getsize(db_path) > 1024:
        logger.info(f"Database exists and is not empty ({os.path.getsize(db_path)} bytes). Skipping restore.")
        return
    ...
    try:
        ...
        latest = files[0]
        logger.info(f"🔄 Restoring latest backup: {latest['name']} (ID: {latest['id']})...")
