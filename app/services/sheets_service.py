    ...
    def sync_bookings_to_sheet(self, bookings: List[Booking]):
        ...
        try:
            ...
            worksheet = self.spreadsheet.add_worksheet(
                title="Все брони", rows=1000, cols=14
            )
        ...
        if len(data) > 0:
            ...
            worksheet.batch_update(
                [{"range": f"A1:N{len(data)}", "values": data}],
    ...
    async def sync_if_needed(self, force: bool = False) -> bool:
        ...
        if not force and self._last_sync_time:
            time_since_last_sync = (
                datetime.now() - self._last_sync_time
            ).total_seconds()
            if time_since_last_sync < self._sync_cache_ttl_seconds:
                logger.debug(
                    f"Skipping sync - last sync was {time_since_last_sync:.1f}s ago (TTL: {self._sync_cache_ttl_seconds}s)"
                )
                return False
