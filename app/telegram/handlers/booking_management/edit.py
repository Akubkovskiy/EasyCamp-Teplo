...
async def show_edit_menu(callback: CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="📅 Даты", callback_data=f"booking:edit_f:{booking_id}:dates"
                )
            ],
            [
                InlineKeyboardButton(
                    text="👤 Имя гостя",
                    callback_data=f"booking:edit_f:{booking_id}:name",
                )
            ],
            [
                InlineKeyboardButton(
                    text="📞 Телефон",
                    callback_data=f"booking:edit_f:{booking_id}:phone",
                )
            ],
            [
                InlineKeyboardButton(
                    text="👥 Кол-во гостей",
                    callback_data=f"booking:edit_f:{booking_id}:count",
                )
            ],
            [
                InlineKeyboardButton(
                    text="💰 Цена", callback_data=f"booking:edit_f:{booking_id}:price"
                )
            ],
            [
                InlineKeyboardButton(
                    text="📊 Статус",
                    callback_data=f"booking:edit_f:{booking_id}:status",
                )
            ],
            [
                InlineKeyboardButton(
                    text="🔙 Назад", callback_data=f"booking:view:{booking_id}"
                )
            ],
        ]
    )
    ...
    if field == "dates":
        ...
        await callback.message.edit_text(
            ...
            reply_markup=build_month_keyboard(
                today.year, today.month, prefix="ebin", min_date=today
            ),
            parse_mode="HTML",
        )
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="🔙 Назад к брони",
                        callback_data=f"booking:cancel_edit:{booking_id}",
                    )
                ]
            ]
        )
        await callback.message.edit_text(
            "👤 Введите новое имя гостя:", reply_markup=keyboard
        )
        ...
        await callback.message.edit_text(
            "📞 Введите новый номер телефона:", reply_markup=keyboard
        )
        ...
        await callback.message.edit_text(
            "👥 Введите количество гостей:", reply_markup=keyboard
        )
        ...
        await callback.message.edit_text(
            "💰 Введите новую стоимость (руб):", reply_markup=keyboard
        )
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="🆕 NEW", callback_data=f"booking:st:{booking_id}:new"
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="✅ CONFIRMED",
                        callback_data=f"booking:st:{booking_id}:confirmed",
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="💰 PAID", callback_data=f"booking:st:{booking_id}:paid"
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="🔔 CHECKING_IN (Заезд сегодня)",
                        callback_data=f"booking:st:{booking_id}:checking_in",
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="🏠 CHECKED_IN (Проживает)",
                        callback_data=f"booking:st:{booking_id}:checked_in",
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="🏁 COMPLETED",
                        callback_data=f"booking:st:{booking_id}:completed",
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="🔙 Назад", callback_data=f"booking:edit:{booking_id}"
                    )
                ],
            ]
        )
        await callback.message.edit_text(
            "📊 Выберите новый статус:", reply_markup=keyboard
        )
...
async def change_ebin_month(callback: CallbackQuery):
    ...
    await callback.message.edit_reply_markup(
        reply_markup=build_month_keyboard(
            year, month, prefix="ebin", min_date=datetime.now().date()
        )
    )
    await callback.answer()
...
async def select_edit_checkin_date(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=build_month_keyboard(
            min_date.year, min_date.month, prefix="ebout", min_date=min_date
        ),
        parse_mode="HTML",
    )
    await callback.answer()
...
async def send_booking_details_refreshed(
    message_or_event, booking_id, edit_instead=False
):
