...
async def settings_users_menu(callback: CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="👮‍♂️ Администраторы", callback_data="users_list:admin")],
        [InlineKeyboardButton(text="🧹 Клининг (Уборщицы)", callback_data="users_list:cleaner")],
...
async def show_users_list(callback: CallbackQuery):
    ...
    buttons.append([InlineKeyboardButton(text="🔙 Назад", callback_data="settings_users")])
...
async def start_add_user(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 Отмена", callback_data=f"users_list:{role_str}")]
        ]),
        parse_mode="HTML"
    )
    await callback.answer()
...
async def process_user_id(message: Message, state: FSMContext):
    ...
    if not message.text and not message.forward_from:
        await message.answer("❌ Пожалуйста, отправьте ID числом или перешлите сообщение.")
        return
...
async def process_user_name(message: Message, state: FSMContext):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 К списку", callback_data=f"users_list:{role_str}")]
    ])
