...
> Это руководство описывает первичную настройку интеграций. Для обновления
> существующего stateful-инстанса сначала выполните `ops/release-gates.md`,
> затем используйте `ops/deploy.md`, `ops/backup.md` и `ops/restore.md`.
> Команды отсюда не являются разрешением на production deploy.
